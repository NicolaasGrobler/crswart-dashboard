  ...
  <section class="container">
    ...
    <div class="tile box">
      <div class="my-block">
        ...
        <p style="font-weight: 700; margin-top: 10px" class="my-block-value">{{ total_lessons }}</p>
      </div>
      <b-tooltip
      ...
      >
        <div class="my-block interactive" style="margin-right: 0px;" @click="$router.push('lessons/mine')">
          <p style="font-size: 20px; font-weight: 700; margin: 0px">
            My Lessons
          </p>
          <p style="font-weight: 700; margin-top: 10px" class="my-block-value">{{ total_made }}</p>
        </div>
      </b-tooltip>
      <b-tooltip
        label="View Lesson"
    ...
    <div class="tile box is-vertical">
      ...
      <div style="display: flex;">
        <div class="my-block" v-for="(subject, index) in subject_totals" :key="index">
        <p style="font-size: 20px; font-weight: 700; margin: 0px">
          {{subject.subject}}
        </p>
        <p style="font-weight: 700; margin-top: 10px" class="my-block-value">{{ subject.occurrence }}</p>
      </div>
      </div>
    </div>
    <!-- Totals by Grade -->
    <div class="tile box is-vertical">
      ...
      <div style="display: flex;">
        <div class="my-block" v-for="(grade, index) in grade_totals" :key="index">
        <p style="font-size: 20px; font-weight: 700; margin: 0px">
          Grade {{grade.grade}}
        </p>
        <p style="font-weight: 700; margin-top: 10px" class="my-block-value">{{ grade.occurrence }}</p>
      </div>
      </div>
    </div>
    <!-- Latest Lesson -->
    <div class="tile is-vertical box">
      ...
      <div style="display: flex; justify-content: space-between">
        <h3 class="date grade">Grade {{ latest_lesson.grade }} - {{ latest_lesson.subject }}</h3>
        <div style="display: flex; align-items: center;">
          ...
          <h3 class="date grade" style="margin-left: 10px !important; background: #640B26;" v-if="new Date(latest_lesson.updated_on).toLocaleDateString('en-GB') != '01/01/1970'">
            Updated {{ new Date(latest_lesson.updated_on).toLocaleDateString("en-GB")}}
          </h3>
        </div>
      </div>
      ...
      <div v-if="JSON.parse(latest_lesson.files.length > 0)" style="margin-top: 20px;">
        <h3>Files</h3>
        <div class="box">
          <div
            class="file"
